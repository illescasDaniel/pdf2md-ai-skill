...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="pdf2md-ai",
        description=(
            "Convert PDF pages to Markdown using MarkItDown and OpenAI vision OCR "
            "for embedded images and scans."
        ),
    )
    parser.add_argument(
        "input_pdf",
...
def main(argv: list[str] | None = None) -> int:
    ...
    with progress:
        ...
        with tempfile.TemporaryDirectory(prefix="pdf2md-ai-") as tmpdir:
            ...
            console.print(
                f"[green]Slice ready:[/green] {text_len} text chars, "
                f"{image_count} embedded image(s)"
            )
            if not args.text_only and image_count:
                console.print(
                    f"[yellow]Expect up to {image_count} OpenAI vision OCR call(s).[/yellow]"
                )
    ...
    console.print(
        f"[green]Done:[/green] {len(markdown):,} chars → {output_path} "

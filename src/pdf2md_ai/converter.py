...
def require_api_key() -> str:
	...
	if not api_key:
		raise RuntimeError("OPENAI_API_KEY is not set. Export your OpenAI API key before running pdf2md-ai.")
	return api_key
...
def install_ocr_progress_hooks(
	...
	global _ocr_hooks_patched
	if _ocr_hooks_patched:
		return
	...
	LLMVisionOCRService.extract_text = extract_text  # pyright: ignore[reportAttributeAccessIssue]
	_ocr_hooks_patched = True
...
def convert_pdf(
	...
	*,
	...
	def on_finish(call_id: int, elapsed: float, chars: int, error: str | None) -> None:
		err = f" error={error}" if error else ""
		log.print(f"[green]✓ OCR call #{call_id} finished in {elapsed:.1f}s ({chars} chars){err}[/green]")
